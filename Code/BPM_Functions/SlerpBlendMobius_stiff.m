...
% 
% Mijk_inv = Mijk^-1;
% % t_ij = gamma_ij/2; t_jk = gamma_jk/2; t_ki = gamma_ki/2;
% t=1/2;
% t_ij = t; t_jk = t; t_ki = t;
% 
% % compute the error term E
% if ~isnan(Mij) 
%      Eij_p = mpower(Mij*Mijk_inv,t_ij);
%     Eij_m = mpower(-Mij*Mijk_inv,t_ij);
% %     Eij_p = mpower(Mij,2/3)*mpower(Mijk_inv,1/3);
% %     Eij_m = mpower(-Mij,2/3)*mpower(Mijk_inv,1/3);
...
% 
% if ~isnan(Mjk) 
%     Ejk_p = mpower(Mjk*Mijk_inv,t_jk);
%     Ejk_m = mpower(-Mjk*Mijk_inv,t_jk);
% %     Ejk_p = mpower(Mjk,2/3)*mpower(Mijk_inv,1/3);
% %     Ejk_m = mpower(-Mjk,2/3)*mpower(Mijk_inv,1/3);
...
% 
% if ~isnan(Mki) 
%     Eki_p = mpower(Mki*Mijk_inv,t_ki);
%     Eki_m = mpower(-Mki*Mijk_inv, t_ki);
% %       Eki_p = mpower(Mki,2/3)*mpower(Mijk_inv,1/3);
% %       Eki_m = mpower(-Mki,2/3)*mpower(Mijk_inv,1/3);
% 
...
% Eijk = (logm(Eij)+logm(Ejk)+logm(Eki))/3;



% w_t = 0.25;
% w_t = 0.5;
% todo: save the logs in vars to save computations
% wl_ij = norm(logm(Eij),'fro'); wl_jk = norm(logm(Ejk),'fro'); 
% wl_ki = norm(logm(Eki),'fro'); 
% 
